...
if isdir(override_dir)
    ...
    function find_artifact_dir()
        return artifact"Vulkan_Headers"
    end
    ...
    function dev_jll()
        ...
        mktempdir() do temp_env
            Pkg.activate(temp_env) do
                Pkg.develop("Vulkan_Headers_jll")
            end
        end
        # Create the override directory
        override_dir = joinpath(Pkg.devdir(), "Vulkan_Headers_jll", "override")
        # Copy the current artifact contents into that directory
        if !isdir(override_dir)
            cp(artifact"Vulkan_Headers", override_dir)
        end
        # Force recompilation of that package, just in case it wasn't dev'ed before
        touch(joinpath(Pkg.devdir(), "Vulkan_Headers_jll", "src", "Vulkan_Headers_jll.jl"))
        @info("Vulkan_Headers_ll dev'ed out to /depot/dev/Vulkan_Headers_jll with pre-populated override directory")
    end
end
